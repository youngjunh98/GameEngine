...
namespace GameEngine
{
	...
	Vector3 Matrix4x4::TransformPosition (Vector3 position)
	{
		Vector4 result = Vector4 (position.m_x, position.m_y, position.m_z, 1.0f) * (*this);
		return Vector3 (result.m_x, result.m_y, result.m_z);
	}

	Vector3 Matrix4x4::TransformDirection (Vector3 direction)
	{
		Vector4 result = Vector4 (direction.m_x, direction.m_y, direction.m_z, 0.0f) * (*this);
		return Vector3 (result.m_x, result.m_y, result.m_z);
	}
