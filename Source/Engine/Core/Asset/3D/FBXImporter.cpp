...
namespace GameEngine
{
	...
	MeshData FBXImporter::Import (uint8* data, const int64 dataSize)
	{
		...
		if (fbxImporter->Initialize (&stream, data, -1, g_fbxManager->GetIOSettings ()))
		{
			...
			if (fbxImporter->Import (fbxScene))
			{
				...
				bool bTriangulateSucceed = fbxGeometryConverter.Triangulate (fbxScene, true);

				FbxNode* fbxRootNode = fbxScene->GetRootNode ();

				if (fbxRootNode && bTriangulateSucceed)
				{
					int32 materialCount = fbxScene->GetMaterialCount ();
					meshData.m_subMeshData.resize (materialCount);

					TraverseNode (fbxRootNode, meshData);
				}
			}
	...
	void FBXImporter::TraverseNode (FbxNode* fbxNode, MeshData& meshData)
	{
			...
			fbxNodeAttribute->GetAttributeType () == FbxNodeAttribute::eMesh)
		{
			...
			for (int32 polygonIndex = 0; polygonIndex < polygonCount; polygonIndex++)
			{
				...
				meshData.m_subMeshData.at (materialId).m_vertices.push_back ({ positions[0], uvs[0], normals[0], tangents[0] });
				meshData.m_subMeshData.at (materialId).m_vertices.push_back ({ positions[1], uvs[1], normals[1], tangents[1] });
				meshData.m_subMeshData.at (materialId).m_vertices.push_back ({ positions[2], uvs[2], normals[2], tangents[2] });
			}
		}

		for (int32 childIndex = 0; childIndex < fbxNode->GetChildCount (); childIndex++)
		{
			TraverseNode (fbxNode->GetChild (childIndex), meshData);
		}
	}
