...
{
	TextureData StbImageImporter::Import (const uint8* fileData, int64 fileSize, bool bForceAlphaChannel, bool bForceGray)
	{
		TextureData texture;
	...
	TextureData StbImageImporter::Resize (const TextureData& textureData, int32 resizedWidth, int32 resizedHeight)
	{
		bool bSucceed = false;
	...
	bool StbImageImporter::IsRGB (const int32 channels)
	{
		return (channels == STBI_rgb) || (channels == STBI_rgb_alpha);
	}

	bool StbImageImporter::IsAlpha (const int32 channels)
	{
		return (channels == STBI_grey_alpha) || (channels == STBI_rgb_alpha);
	}

	bool StbImageImporter::IsOpaque (const int32 channels)
	{
		return (channels == STBI_grey) || (channels == STBI_rgb);
	}

	ERenderingResourceFormat StbImageImporter::GetFormat (const int32 channels, const int32 bytes, const bool bLinearColor)
	{
